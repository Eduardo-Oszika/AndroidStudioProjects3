...
public class MeuAdapterPersonagem extends RecyclerView.Adapter<MeuAdapterPersonagem.ViewHolder> {
    private List<Personagem> itens;

    public MeuAdapterPersonagem(List<Personagem> itens) {
        this.itens = itens;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (item.id != null) {
            textId.setText(String.valueOf(item.id).trim());
        }
        TextView texto = holder.getTextName();
        texto.setText(item.name.trim());
        TextView textoPlayer = holder.getTextPlayer();
        if (position > 0) {
            textoPlayer.setText("Android");
        } else {
            textoPlayer.setText("Jogador");
        }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private TextView textPlayer, textName, textId;
        private ImageView imageView;

        public ViewHolder(@NonNull View itemView) {
            super(itemView);
            textPlayer = itemView.findViewById(R.id.playerName);
            textName = itemView.findViewById(R.id.tvItem);
            textId = itemView.findViewById(R.id.tvId);
            imageView = itemView.findViewById(R.id.imgItem);
        }

        public TextView getTextPlayer() {
            return textPlayer;
        }

        public void setTextPlayer(TextView textPlayer) {
